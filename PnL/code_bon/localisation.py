...
@author: thgerm
"""
#https://clairelabitbonis.gitlab.io/post/tp-localisation-pikachu-fr/
...
def transform_and_draw_model(edges_Ro, intrinsic, extrinsic, fig_axis):
    ...
    P1_c = matTools.transform_point_with_matrix(extrinsic, edges_Ro[:,:3])
    P2_c = matTools.transform_point_with_matrix(extrinsic, edges_Ro[:,3:])
    [u_1,v_1] = perspective_projection(intrinsic,P1_c)
    [u_2,v_2] = perspective_projection(intrinsic,P2_c)
...
def calculate_normal_vector(p1_Ri, p2_Ri, intrinsic):
    ...
    #normal_vector = np.zeros((len(p1_Ri),))

    inv_intrinsic = np.linalg.inv(intrinsic)
    P1_Rc = np.dot(inv_intrinsic,p1_Ri)
    P2_Rc = np.dot(inv_intrinsic,p2_Ri)
    l1 = P1_Rc#-P0=0
    l2 = P2_Rc-P1_Rc
...
def calculate_error(nb_segments, normal_vectors, segments_Rc):
    ...
    for p in range(nb_segments):
        ...
        err = err + np.power(np.dot(normal_vectors[p],segments_Rc[p,:3]),2)
        err = err + np.power(np.dot(normal_vectors[p],segments_Rc[p,3:]),2)
...
if __name__ == "__main__":
    ...
    ax7.set_xlim(0, 720)
    plt.imshow(image)
    transform_and_draw_model(model3D_Ro_final, intrinsic_matrix, extrinsic_matrix, ax7)
    plt.show(block=True)