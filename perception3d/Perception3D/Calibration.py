...
class CalibrationBase:
    ...
    def detect(self, frame, refine=False):
        ...
        if self.patternType == 'chessboard':
            ...
            patternFound,corners = cv.findChessboardCorners(frame, (8, 5), None)
        ...
        #     centers = cv2.findCirclesGrid(frame, (8, 5), flags=cv2.CALIB_CB_SYMMETRIC_GRID)
        # elif self.patternType == 'asymmetric_circles':
        #     # Implement findCirclesGrid here
        #     centers = cv2.findCirclesGrid(frame, (8,5), flags=cv2.CALIB_CB_ASYMMETRIC_GRID)
        #
...
class MonoCalibration(CalibrationBase):      
    ...
    def calibrate(self, framesPath = 'results/acquired'):
        ...
        self.rms, self.cameraMatrix, self.distCoeffs, self.rvecs, self.tvecs, Intrinsics, Extrinsics, self.perViewErrors = cv.calibrateCameraExtended(objectPoints, imgPoints, imageSize, None, None )
